...
export type AugmentedResponse<T = unknown> = ky.ResponsePromise<T> & {
...
export type AugmentedKyInstance = Omit<ky.KyInstance, 'extend' | 'get' | 'post' | 'put' | 'delete' | 'patch' | 'head' | 'options'> & {
  ...
  extend: (defaultOptions?: ky.Options | ((parentOptions: ky.Options) => ky.Options)) => AugmentedKyInstance
}
